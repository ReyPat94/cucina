...
<body>
	...
	<br>
	<a href=""><button>I corsi ai quali sei iscritta/o</button></a>
	<br> Visualizza i corsi per:
	<select id="options">
		...
		function allYears() {
			$.getJSON("JSONuserpageAnni", function(anniList) {
				$.each(anniList,
						function(index, item) {
							$(
									"<button type='button' onclick = 'allEdizioniAnni()' name = 'choice' value = "
											+ item + " >").text(item).appendTo(
									"#anno");
						})
			});
		}
	...
	<script>
		function allEdizioniAnni() {
			$.getJSON("JSONedizioniAnni?anno=2010", function(edizioniList) {
				$.each(edizioniList, function(index, item) {
					var trow = ("<tr>").appendTo($("#edizioni"))
					$("<td>").text(item.edizione.corso.titolo).appendTo(trow);
					$("<td>").text(item.edizione.corso.numeroMaxPartecipanti)
							.appendTo(trow);
					$("<td>").text(item.edizione.corso.costo).appendTo(trow);
					$("<td>").text(item.edizione.dataInizio).appendTo(trow);
					$("<td>").text(item.edizione.durata).appendTo(trow);
					$("<td>").text(item.edizione.aula).appendTo(trow);
					$("<td>").text(item.edizione.docente).appendTo(trow);
				})
			});
		}
	</script>
