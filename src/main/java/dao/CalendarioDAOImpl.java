...
public class CalendarioDAOImpl implements CalendarioDAO {
	...
	public ArrayList<Edizione> select(int idCaregotia, boolean future) throws SQLException {
		...
		if (future) {
			pstmt = conn.prepareStatement(
					"SELECT * from calendario JOIN catalogo ON(id_corso) WHERE id_categoria = ? and dataInizio > ?");
	...
	public ArrayList<Edizione> select(String idUtente, boolean future) throws SQLException {
		...
		if (future) {
			pstmt = conn.prepareStatement(
					"SELECT * from calendario JOIN iscritti ON(id_edizione) WHERE user_id = ? AND dataInizio > ? ");
			java.util.Date today = new java.util.Date();
			Date todaySQL = new Date(today.getTime());
			pstmt.setDate(2, todaySQL);
		} else {
	...
	/*
	 * Numero massimo di partecipanti a un'edizione in base al corso a cui
	 * l'edizione appartiene se la query non viene eseguita ritorna 0
	 */
	@Override
